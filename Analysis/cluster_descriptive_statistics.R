...
other <- c('Cluster',
           ...
           'Maternal Smoking',
           'Anxiety',
           'Able To Confide', ### WHERE DID THIS GO HAHAHA
           'Neuro Score',
           'Distress Score',
           'Sleep Score',
           'Diet Score',
...
## pvalues - continuous variables: kurskal-wallis test, categorical: chi-square test
## Sociodemographic
table1_sociodemographic <- CreateTableOne(sociodemographic,
                         strata = 'Cluster',
                         data = ukb_clustered)
table1_sociodemographic
## Environmental
table1_environmental <- CreateTableOne(environmental,
                                          strata = 'Cluster',
                                          data = ukb_clustered)
table1_environmental
## Other
table1_other <- CreateTableOne(other,
                               strata = 'Cluster',
                               data = ukb_clustered)
table1_other
...
### Honestly not sure how to save it as an actual image
...
table1(~ `Alcohol Intake` + `Alcohol 10 Yrs` + `Smoking Status` + `Pack Years` + `Breastfed` + 
         `Maternal Smoking` + Anxiety + `Neuro Score` + `Distress Score` + `Sleep Data` +
         `Diet Score` + `Met Score` # + `Able to Confide`
