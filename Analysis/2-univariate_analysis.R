# Runs univariate analysis and generates tables to use in analysis_plots
# univariate_pvalues.rds
# manhattan_table.rds
# univariate_log.rds
# manhattan_log.rds
# figure_names
...
#   ukb_analysis[,i] <- scale(ukb_analysis[,i])
# }
...
# CHECK
# model1 <- glm(cluster_3 ~ coast_distance + age + sex + ethnic_background, data = ukb_analysis, family = 'binomial')
# summary(model1)
...
#max(univariate_log, na.rm = T) # 290.4868
...
figure_names <- data.frame(Variable = c(sort(unique(manhattan_log$Variable)), "smoking_status_Previous",
                           ...
                           figure_variable = c("Distress score (1+)",
                                               ...
                                               "Other ethnic backgrounds", "Sex (Male)", "Smoking (current)", "Smoking (previous)",
                                               "Vehicles (1)", "Vehicles (2+)", "Anxiety"))
