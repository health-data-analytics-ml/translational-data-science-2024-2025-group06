...
manhattan_log %>% filter(Cluster == 1) %>% ggplot(aes(x = Subgroup, y = pval, label = figure_variable)) +
  # Show all points
  ...
  scale_color_manual(values = rep(c("royalblue4", "steelblue1"), 22 )) +
  ...
  labs(title = "Cluster 1: Manhattan Plot of Exposure P-values") + 
  ylab(expression(-log[10](italic('p')))) +
  xlab("Exposure Subgroup") +
  ylim(0,54)

ggsave("c1_manhattan.png", path = '/rds/general/project/hda_24-25/live/TDS/Group06/Scripts/Visualisations/Analysis_visuals/Manhattan_plots',
       width = 8, height = 5)
...
ggplot(barplot, aes(fill=Subgroup, y=Count, x=Cluster)) + 
  ...
  #scale_fill_manual(values = c("red", "seagreen3", "grey", "purple", "blue"))
...
p <- ggplot(selected_longer, aes(x = fct_inorder(figure_variable), y = cluster_name, fill = `Selection Proportion`)) +
  geom_tile(color = "white", lwd = 0.5, linetype = 1) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 6)) +
  scale_fill_gradient(low = "white", high = "steelblue") + 
  labs(title = 'Tilemap of Variable Selection Proportions',
       subtitle = 'Stability Selection LASSO') +
  xlab('') +
  theme(plot.title = element_text(hjust = 0)) +
  theme(legend.position = 'none') +
  ylab('')
p
ggsave('/rds/general/project/hda_24-25/live/TDS/Group06/Scripts/Visualisations/Analysis_visuals/sel_prop_tilemap.png', plot = p, width = 8, height = 4, dpi = 300)
...
p <- ggplot(log_output, aes(x = fct_inorder(figure_variable), y = cluster_name, fill = as.factor(OR_colour))) +
  geom_tile(color = "black", lwd = 0.1, linetype = 1) + 
  geom_text(aes(label = Odds_ratio), color = "black", size = 3) +  
  ...
  ylab('')
p
ggsave('/rds/general/project/hda_24-25/live/TDS/Group06/Scripts/Visualisations/Analysis_visuals/log_regression_tilemap.png', plot = p, width = 11, height = 5, dpi = 300)
